...
describe('Inventory Management (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    const storeManager = await createUser(
  ...
  it('should return 403 if user role is not a store manager or admin', async () => {
  ...
  it('should return 403 if store_manager not associated with store', async () => {
    const anotherStore = await createStore({});
    const requestBody = new AdjustInventoryDto();
    requestBody.storeId = anotherStore.id;
    requestBody.bookId = BOOK.id;
    requestBody.quantityChange = 10;
    return request(APP.getHttpServer())
      .put(`/store/adjust-inventory`)
